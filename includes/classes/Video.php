...
class Video {
    private $connect, $sqlData, $entity;
    ...
    public function __construct($connect, $input) {
        ...
        if(is_array($input)) {
            ...
            $query = $this->connect->prepare("SELECT * FROM videos WHERE id=:id");
        ...
        $this->entity = new Entity($connect, $this->sqlData["entityId"]);
    }
    ...
    public function incrementViews(){
       $query =  $this->connect->prepare(" UPDATE videos SET views=views+1 WHERE id=:id ") ;
    ...
    public  function isInProgress($username){
       ...
       $query = $this->connect->prepare("SELECT * FROM videoProgress
